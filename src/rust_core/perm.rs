...
impl Perm {
    ...
    pub fn pattern_details(&self) -> PattDetails {
        ...
        self.val
            .iter()
            .zip(self.left_inf_sup())
            .map(|(val, left_inf_sup)| {
        ...
        for (idx, val) in self.val.iter().enumerate() {
            match left_floor_ceiling {
                ...
                Some((floor, _)) if val < floor => {
                    ...
                    left_floor_ceiling = Some((val, left_floor_ceiling.unwrap().1));
                    //TODO: this is ugly
                }
    ...
    pub fn insert(&self, index: usize, elmt: Element) -> Self {
        //can do this in a more idiomatic style once slice_take is stable
        //see https://github.com/rust-lang/rust/issues/62280
        ...
        for i in 0..index {
            let val = self.val[i];
            elmts.push(if val < elmt { val } else { val + 1 });
        }
    ...
    pub fn remove(&self, index: usize) -> Self {
        let elmt = self.val[index];
        let val = self.val.iter().enumerate().filter(|(idx, _)| idx != &index).map(|(idx, val)| {
            use std::cmp::Ordering::*;
            match val.cmp(&elmt) {
                Less => *val,
                Greater => val - 1,
                Equal => panic!("Invalid Permutation: elements with value {val} at indecies {index} and {idx}."),
            }
        }).collect();
    ...
    pub fn append(&self, elmt: Element) -> Perm {
        self.insert(self.len(), elmt)
    }
}
...
impl<T: Patt + Sized> Iterator for IntoPattIter<T> {
    ...
    fn next(&mut self) -> Option<Self::Item> {
        ...
        let (mut i, mut next_occ) = match self.curr.clone() {
            Some(mut occ) => (occ.pop().unwrap() + 1, occ),
            None => (0, vec![]),
        };
